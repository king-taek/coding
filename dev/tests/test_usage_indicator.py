"""상태바 GPU '가동/대기' 표시의 기반 — 가속 유닛 활동 추적 단위 테스트.

Intel GPU 의 실제 점유율(%)은 이식성 있게 얻을 수 없어, OpenVINO 추론이
...
def test_unit_idle_when_never_marked():
    # 한 번도 마킹하지 않은 태그는 대기.
    assert ov.unit_busy("NEVER_USED_TAG") is False
...
def test_window_boundary():
    ov.mark_unit_active("GPU")
    time.sleep(0.05)
    assert ov.unit_busy("GPU", window=1.0) is True
    assert ov.unit_busy("GPU", window=0.01) is False
