...
class EfficiencyScheduler(QThread):
    ...
    # -- fusion 한 ref 처리 -------------------------------------------------
    def _fuse_ref(self, ref, vals, built, ref_emb, by_path, vfmap=None) -> Optional[list]:
        """임베딩 성공 시 fusion 결과 [(val_path, score), …] 내림차순, 실패 시 None.

        ``vfmap`` (슬롯 val Feature 캐시) 가 주어지면 고전 재채점에서 재추출 생략."""
        if built is None:
            return None
    ...
    def _rerank_one(self, r, vals, built, ref_emb, by_path, vfmap):
        """ref 1장 재채점(직렬 버전과 동일 결과) — 풀 워커에서 병렬 실행."""
        try:
            cands = self._fuse_ref(r, vals, built, ref_emb, by_path, vfmap)
        except Exception:
            cands = None
        if cands is None:                            # 고전 폴백(임베딩 실패/미가용)
            cc = score_ref_classical(r, vals, threshold=self._threshold,
                                     cfg=self._cfg, val_features=vfmap,
                                     stop_cb=self._stop.is_set)
            cands = [(c.item.path, float(c.score)) for c in cc]
        return cands

    def _consume_slot(self, slot, refs, vals, built, ref_emb) -> None:
        """슬롯 val 특징 1회 캐시 + ref들을 멀티코어로 병렬 재채점(결과는 직렬과 동일)."""
        if not refs:
            self._finished_slots += 1
            self.signals.slot_finished.emit(slot, self._finished_slots, self._total_slots)
            return
        vfmap = self._slot_val_features(vals)        # ② 1회 병렬 추출(공유)
        by_path = {Path(v.path): v for v in vals}
        # ① ref별 재채점을 코어 수만큼 동시 실행 — 채점은 ref끼리 독립이라 결과 동일.
        futs = {self._pool.submit(self._rerank_one, r, vals, built, ref_emb, by_path,
                                  vfmap): r for r in refs}
        for fut in as_completed(futs):
            ...
            try:
                ...
                cands = []
            self._results[(slot, Path(r.path))] = cands
            with self._plock:
    ...
    def _run(self) -> None:
        ...
        try:
            if backend is None:
                ...
                # GPU 임베딩(생산자 스레드) ∥ CPU 병렬 재채점(소비자) — 동시 가동.
            ...
            self._pool.shutdown(wait=not self._stop.is_set())
        self.signals.finished.emit()
