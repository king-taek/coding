...
# 탤리 옆 탐색 버튼 — 다음 예외(허용 초과·매치 없음·매치 실패) 행으로 데려간다.
# 행을 숨기는 필터가 아니라 순수 이동이다(끝에 닿으면 처음으로 돈다).
BTN_NEXT_ISSUE = "다음 예외로"
# 예외가 하나도 없으면 이 버튼은 갈 곳이 없다 — 눌러도 아무 일이 없는 '먹은 클릭'
# 대신 비활성으로 두고, 왜 잠겼는지를 툴팁이 말한다(벌크 선택·슬롯 묶기와 같은 규약).
BTN_NEXT_ISSUE_NONE_TOOLTIP = "예외가 없습니다 — 모든 행이 일치입니다."
