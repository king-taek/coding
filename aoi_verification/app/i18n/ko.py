...
# `.t.` 사진 — 이번 검증에 포함할지 **세션마다 묻는다**(사용자 요청).
# ★ '항상 뺀다' 와 '항상 넣는다' 사이를 세 번 오간 자리다(`models.slot.is_ignored_name`
#   주석).  정답이 자재마다 다르다는 것이 결론이라, 코드가 정하지 않고 근거(예시 사진)
#   와 함께 묻는다.
T_PHOTO_ASK_TITLE = "이 사진들도 포함할까요?"
T_PHOTO_ASK_HEAD_FMT = "이름에 ‘.t.’ 가 들어간 사진이 {n} 장 있습니다"
T_PHOTO_ASK_BODY = (
    "아래는 그중 한 장입니다. 이런 사진도 이번 검증에 포함할지 정해 주세요.\n"
    "‘제외하고 진행’ 을 고르면 이 사진들은 스캔·썸네일·매칭에서 모두 빠집니다."
)
T_PHOTO_ASK_YES = "포함하고 진행"
T_PHOTO_ASK_NO = "제외하고 진행"
T_PHOTO_ASK_NO_PREVIEW = "미리보기를 읽을 수 없습니다"
