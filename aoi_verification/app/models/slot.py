...
def has_t_token(name: str) -> bool:
    """점으로 구분된 ``t`` 토큰을 가진 이름인가.

    예: ``-86955.68631.t.1.jpg`` → 확장자를 한 단계 뗀 stem ``-86955.68631.t.1`` 을
    ``.`` 으로 나눈 토큰 ``[-86955, 68631, t, 1]`` 에 정확히 ``t`` 가 있다.
    같은 자리에 ``c`` 가 오는 사진(실물 다수: ``272646.165679.c.1000203959.2.jpeg``)은
    해당하지 않는다.
