...
- 문서는 `docs/`. 사용자가 실행하는 스크립트는 `scripts/`(run_aoi*.bat·run_this_before.py·
  update_app.bat). 빌드/내부 도구는 `scripts/internal/`(*.spec·build_windows·make_portable·
  verify_no_forbidden). 경로 바꾸면 이들을 호출하는 곳(run_this_before·빌드 bat·README·문서)도 함께 고친다.
- **`dev/` = 사용자가 직접 건드리지 않는 개발 전용 모음.** `dev/tests/`(테스트)·`dev/bench결과/`
  (실측 데이터)·`dev/양식.xlsx`(엑셀 출력 템플릿)를 한곳에 모았다. 옮길 때 함께 고칠 참조:
  - 테스트 경로: `pytest.ini` 의 `testpaths = dev/tests` (※ `pytest.ini` 자체는 루트 앵커라
    이동 금지 — `python -m pytest` 가 루트에서 testpaths 로 찾는다).
  ...
  - 양식 템플릿: `paths.template_path()` 가 `dev/양식.xlsx` 를 1순위로 찾는다. 포터블 빌드는
    `make_portable.bat`/`아이.spec`/`updater` 가 `dev/양식.xlsx` → 앱 루트 `양식.xlsx` 로 복사한다.
