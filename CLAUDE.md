...
- 회사에서 **금지한 외부 패키지 매니저/IDE 계열 도구**를 코드·문서·의존성에 도입하지 않는다
  (구체 목록·패턴은 `scripts/verify_no_forbidden.py` 와 테스트 `tests/test_no_spyder_conda.py`
  가 보유). 금지 키워드를 문서에 적기만 해도 가드가 실패하니, 이름을 직접 쓰지 말고 가드를 참조한다.
- 커밋 전 `python scripts/verify_no_forbidden.py` 가 통과해야 한다(`run_this_before.py` 도 실행).
