...
완성된 PDF 두 개(``docs/사용설명서.pdf`` 핵심판 · ``docs/상세설명서.pdf`` 완전판)만
내려간다.

★ 여기서 찍은 ``*.png``/``*.jpg`` 는 **저장소에 남지 않는다.**  PDF 를 굽고 나면
  ``make_manual_pdf.py`` 가 치우고(`sweep_captures`) 파일명만 ``사진목록.txt`` 로
  남긴다 — 이 스크립트가 언제든 같은 이름으로 다시 찍어 주기 때문이다.  그러니
  설명서를 고칠 때는 **늘 이 스크립트부터** 돌려야 한다.

여기서 찍은 캡처는 **두 문서가 함께 쓴다**:
